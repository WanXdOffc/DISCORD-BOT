  ...
  setupRoutes() {
    ...
    this.app.post('/api/guild/:guildId/settings', this.requireAuth, async (req, res) => {
      ...
      try {
        ...
        if (settings.economy) {
          guildData.economy = {
            enabled: guildData.economy?.enabled !== undefined ? guildData.economy.enabled : true,
            currency: guildData.economy?.currency || { name: 'coins', symbol: '🪙' },
            daily: {
              ...(guildData.economy?.daily || { enabled: true, amount: 100, streak: true }),
              ...(settings.economy.daily || {})
            },
            work: guildData.economy?.work || { enabled: true, cooldown: 3600000, minAmount: 50, maxAmount: 200 }
          };
        }
